...
function hasCookie(){return/('.ecs-dms-wrap').forEach(function(wrap){wrap.classList.toggle('is-alt',isAlt);var display=wrap.dataset.display;if(display==='toggle'){var btn=wrap.querySelector('.ecs-dms-btn');if(btn){btn.classList.toggle('is-active',isAlt)}}else if(display==='dual'){wrap.querySelectorAll('.ecs-dms-btn').forEach(function(btn){var isActive=btn.dataset.scheme===ALT_VALUE'is-active',isActive);btn.setAttribute('aria-pressed',isActive'true':'false')})}else if(display==='dropdown'){var select=wrap.querySelector('.ecs-dms-select');if(select){select.value=isAlt'default'}}})}
...
function init(){syncAllWidgets();var cfg=window.ecsSchemeConfig||{};if(cfg.systemAuto){var mq=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)');if(mq){mq.addEventListener('change',function(e){if(!hasCookie()){applyScheme(e.matches'default');syncAllWidgets()}})}}
